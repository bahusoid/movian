	    ...
	    struct token *t)
{
  ...
  if (strstr(glw_get_name(w), "home.view"))
  {
    /* Try to find a nearby widget that is associated with focus so we can
     * map captioned labels to the widget that was focused earlier. This is
     * useful when focus is set before child label widgets are created. */
    glw_t *assoc = NULL;
    glw_t *candidate_focuspath = NULL;
    glw_t *best_focusweight = NULL;
    glw_t *p = w->glw_parent;

    /* If the root has a current focus, attempt to match an ancestor by
     * the focused widget's path. This lets us detect when the focused
     * container (e.g. container_z @ file:line) appears in the focused
     * widget's path even though a child container (e.g. container_y)
     * exists between it and the label. */
    /* Prefer gr_last_focus (set when focus was changed) so captions set
     * shortly after focus will map to the widget that was focused. Fall
     * back to gr_current_focus if last_focus is not set. */
    glw_t *root_focused = NULL;
    if(w->glw_root) {
      root_focused = w->glw_root->gr_last_focus ? w->glw_root->gr_last_focus : w->glw_root->gr_current_focus;
    }

    /* Strong match: try to find an ancestor that matches the focused
     * widget by class, file and line. This ensures we map to the exact
     * container (e.g. container_z) that was focused. */
    glw_t *class_only_match = NULL;
    if(root_focused) {
      const char *fclass = root_focused->glw_class && root_focused->glw_class->gc_name ?
        root_focused->glw_class->gc_name : NULL;
      rstr_t *ffile = root_focused->glw_file;
      int fline = root_focused->glw_line;

      for(p = w->glw_parent; p != NULL; p = p->glw_parent) {
        if(fclass && p->glw_class && p->glw_class->gc_name && !strcmp(p->glw_class->gc_name, fclass)) {
          /* exact file+line match */
          if(ffile && p->glw_file && rstr_get(ffile) && !strcmp(rstr_get(ffile), rstr_get(p->glw_file)) && p->glw_line == fline) {
            assoc = p;
            break;
          }
          /* remember class-only match as fallback */
          if(class_only_match == NULL)
            class_only_match = p;
        }
      }
    }

    /* If strong match by focused widget failed, continue with other heuristics */
    for(p = w->glw_parent; p != NULL; p = p->glw_parent) {
      if(p->glw_flags & GLW_IN_FOCUS_PATH)
        candidate_focuspath = p; /* keep overwriting to end up with highest ancestor */
      if(p->glw_focus_weight > 0) {
        if(best_focusweight == NULL || p->glw_focus_weight > best_focusweight->glw_focus_weight)
          best_focusweight = p;
      }
    }
    ...
    if(assoc == NULL) {
      if(class_only_match != NULL)
        assoc = class_only_match;
      else if(candidate_focuspath != NULL)
        assoc = candidate_focuspath;
      else if(best_focusweight != NULL)
        assoc = best_focusweight;
    }
    if(assoc) {
      char lab_addr[32];
      char assoc_addr[32];
      char assoc_cap[256];
      char pathbuf[1024];
      snprintf(lab_addr, sizeof(lab_addr), "%s", get_addr_str(w));
      snprintf(assoc_addr, sizeof(assoc_addr), "%s", get_addr_str(assoc));
      snprintf(assoc_cap, sizeof(assoc_cap), "%s", get_catpion(assoc));
      snprintf(pathbuf, sizeof(pathbuf), "%s", glw_get_path(w));
      TRACE(TRACE_DEBUG, "GLW", "set_caption: widget %s caption=%s associated=%s assoc_caption=%s type=%d path=%s",
          lab_addr, str ? str : "(null)", assoc_addr, assoc_cap, type, pathbuf);
    } else {
      char lab_addr[32];
      char pathbuf[1024];
      snprintf(lab_addr, sizeof(lab_addr), "%s", get_addr_str(w));
      snprintf(pathbuf, sizeof(pathbuf), "%s", glw_get_path(w));
      TRACE(TRACE_DEBUG, "GLW", "set_caption: widget %s caption=%s type=%d path=%s",
          lab_addr, str ? str : "(null)", type, pathbuf);
    }
  }
