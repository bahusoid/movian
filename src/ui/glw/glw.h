...
void glw_osk_open_default(glw_root_t *gr, const char *title, const char *input,
                          glw_t *w, int password);
...
static inline const char *get_cap_str(const char *c, void *p) {
  static char buf[160];
  uintptr_t v = (uintptr_t)p;
  snprintf(buf, sizeof(buf), "%s (0x%05lx)",c, (unsigned long)(v & 0xFFFFF));
  return buf;
}

static inline const char *get_addr_str(void *p) {
  static char buf[16]; // enough for "0x" + 5 hex digits + '\0'
  uintptr_t v = (uintptr_t)p;
  snprintf(buf, sizeof(buf), "0x%05lx", (unsigned long)(v & 0xFFFFF));
  return buf;
}

static inline const char *get_catpion(glw_t *w) {
  static char buf[256];

  if (!w)
    return "(null)";

  /* 1) If the widget itself exposes text return it */
  if (w->glw_class && w->glw_class->gc_get_text) {
    const char *text = w->glw_class->gc_get_text(w);
    if (text && text[0])
      return text;
  }

  /* 2) Search direct children for a caption-providing widget (common pattern
   * where caption is a sibling/child label widget). */
  {
    glw_t *c;
    TAILQ_FOREACH(c, &w->glw_childs, glw_parent_link) {
      if (c->glw_class && c->glw_class->gc_get_text) {
        const char *ct = c->glw_class->gc_get_text(c);
        if (ct && ct[0])
          return ct;
      }
    }
  ...
  /* 3) Check parent (the caption might live on the parent) */
  if (w->glw_parent) {
    glw_t *p = w->glw_parent;
    if (p->glw_class && p->glw_class->gc_get_text) {
      const char *pt = p->glw_class->gc_get_text(p);
      if (pt && pt[0])
        return pt;
    }

    /* 4) Search siblings (other children of the parent) */
    glw_t *s;
    TAILQ_FOREACH(s, &p->glw_childs, glw_parent_link) {
      if (s == w)
        continue;
      if (s->glw_class && s->glw_class->gc_get_text) {
        const char *st = s->glw_class->gc_get_text(s);
        if (st && st[0])
          return st;
      }
    }
  ...
  /* 5) Nothing found - return a useful fallback describing the widget */
  snprintf(buf, sizeof(buf), "%s %s",
           w->glw_class && w->glw_class->gc_name ? w->glw_class->gc_name : "<widget>",
           get_addr_str(w));
  return buf;
}
